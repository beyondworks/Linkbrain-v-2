...
### 목표
1. **다중 링크 분석 UX 개선**: 연속적인 링크 분석을 위한 자동 입력창 초기화
2. **무한 스크롤 구현**: 'Load More' 버튼 대신 자동 로딩
3. **상세페이지 헤더 고정**: 스크롤 시에도 뒤로가기/삭제 버튼 접근 가능

### 주요 변경 사항

#### 1. 다중 링크 분석 UX (`Hero.tsx`, `FloatingSearchButton.tsx`)
- **2초 애니메이션**: 종이비행기 2바퀴 회전 (720도)
- **자동 초기화**: 1.5초에 페이드 아웃 시작, 2초에 입력창 완전 초기화
- **백그라운드 분석**: API 호출이 완료되기 전에 다음 링크 입력 가능
- **토스트 위치 변경**: `bottom-right`로 이동, 퀵 버튼과 겹치지 않도록 offset 적용

#### 2. 무한 스크롤 (`ClipGrid.tsx`)
- **IntersectionObserver** 활용으로 자동 로딩 구현
- 스크롤 하단 도달 시 6개씩 추가 로딩
- 로딩 스피너 표시로 사용자 피드백 제공

#### 3. 상세페이지 헤더 고정 (`ClipDetail.tsx`)
- **fixed 포지셔닝**: 스크롤 시에도 헤더 상단 고정
- **반응형**: 데스크톱에서 사이드바(100px) 고려한 위치 조정
- **백드롭 블러**: 콘텐츠와 헤더 구분을 위한 시각적 효과
- **삭제 버튼**: 모바일/데스크톱 모두에서 표시
