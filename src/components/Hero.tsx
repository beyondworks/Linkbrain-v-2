  ...
  return (
    <div className="w-full flex flex-col items-center justify-center pt-32 md:pt-40 pb-20 px-6 text-center relative overflow-hidden">
      ...
      <motion.div
        ...
        className="w-full max-w-[800px] relative rounded-[40px] md:rounded-full z-10 mx-6 md:mx-auto select-none"
      >
        <div className="relative w-full h-[80px] md:h-[76px] bg-white dark:bg-[#1e1e1e] rounded-[40px] md:rounded-full flex items-center px-6 md:px-8 overflow-visible border border-transparent border-gray-100 dark:border-gray-800 shadow-sm md:shadow-none">
          <input
            type="text"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            placeholder={language === 'KR' ? "URL을 여기에 붙여넣으세요" : "Paste any URL here"}
            className="w-full h-full bg-transparent border-none outline-none text-[17px] md:text-xl text-[#3d3d3d] dark:text-white placeholder-[#c5c5c5] transition-opacity duration-500 pr-16 md:pr-14"
            style={{ opacity: isFading ? 0 : 1 }}
            onKeyDown={(e) => e.key === 'Enter' && handleSearch()}
            disabled={isProcessing}
          />

          {/* Mobile Button (Green Circle) */}
          <motion.button
            onClick={handleSearch}
            whileHover={!isProcessing ? { scale: 1.05 } : {}}
            whileTap={!isProcessing ? { scale: 0.95 } : {}}
            className="md:hidden absolute right-2 top-1/2 -translate-y-1/2 w-[64px] h-[64px] flex items-center justify-center bg-[#21DBA4] rounded-full shadow-md"
            disabled={isProcessing}
          >
          ...
          {/* Desktop Button (Original Paper Plane) */}
          <motion.button
            onClick={handleSearch}
            whileHover={!isProcessing ? { scale: 1.05 } : {}}
            whileTap={!isProcessing ? { scale: 0.95 } : {}}
            className="hidden md:flex absolute right-3 top-1/2 -translate-y-1/2 w-[54px] h-[54px] items-center justify-center"
            disabled={isProcessing}
          >
            <div className="relative w-full h-full group cursor-pointer flex items-center justify-center">
              <motion.div
