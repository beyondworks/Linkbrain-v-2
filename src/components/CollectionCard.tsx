...
const CollectionCard = ({ name, count, color = "bg-[#21DBA4]", updatedAt, onDelete, onEdit, language = 'KR' }: CollectionCardProps) => {
  return (
    <div className="w-full bg-white rounded-[24px] p-6 border border-[#E0E0E0] shadow-sm hover:shadow-md transition-all cursor-pointer group flex flex-col justify-between h-[180px]">
      ...
      <div className="flex items-start justify-between">
        ...
        <div onClick={(e) => e.stopPropagation()}>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="bg-white rounded-xl border border-gray-100 shadow-lg p-1 z-50">
                    <DropdownMenuItem
                        className="cursor-pointer rounded-lg hover:bg-gray-50 text-sm text-gray-600 px-3 py-2 outline-none transition-colors"
                        onClick={(e) => {
                            e.stopPropagation();
                            onEdit?.();
                        }}
      ...
      <div className="flex flex-col gap-1 mt-2">
        <h3 className="text-[#3d3d3d] text-[20px] font-semibold group-hover:text-[#21DBA4] transition-colors truncate">
            {name}
        </h3>
