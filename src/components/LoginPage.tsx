...
interface LoginPageProps {
  onNavigate: (view: 'signup' | 'clips') => void;
  onLoginSuccess: () => void;
  language?: 'KR' | 'EN';
}

const LoginPage = ({ onNavigate, onLoginSuccess, language = 'KR' }: LoginPageProps) => {
      ...
      <div className="w-full max-w-[420px] bg-white rounded-[32px] shadow-xl p-8 md:p-12">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <h1 className="text-[32px] font-bold text-[#3d3d3d]">
            {language === 'KR' ? '다시 돌아오셨군요' : 'Welcome back'}
          </h1>
          <p className="text-[#959595] text-lg mt-2">
            {language === 'KR' ? '계정 정보를 입력하세요' : 'Please enter your details'}
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="flex items-center justify-end">
            <button type="button" className="text-sm font-medium text-[#21dba4] hover:text-[#1cc492]">
              {language === 'KR' ? '비밀번호를 잊으셨나요?' : 'Forgot password?'}
            </button>
          </div>

          <Button
            type="submit"
            className="w-full h-[56px] rounded-2xl bg-[#21DBA4] hover:bg-[#1cc492] text-white text-lg font-bold shadow-lg shadow-[#21dba4]/20"
            disabled={isLoading}
          >
            {isLoading ? (
              <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ) : (
              language === 'KR' ? '로그인' : 'Sign In'
            )}
          </Button>
        </form>
        ...
        <div className="relative my-8">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-white px-2 text-[#959595]">
              {language === 'KR' ? '또는 다음으로 계속' : 'Or continue with'}
            </span>
          </div>
        </div>
        ...
        <p className="mt-8 text-center text-[#959595]">
          {language === 'KR' ? '계정이 없으신가요? ' : "Don't have an account? "}
          <button
